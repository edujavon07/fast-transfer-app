...
# ==========================================
# If running on Android, request permissions and set path to the Downloads folder
if platform == 'android':
    from android.permissions import request_permissions, Permission
    request_permissions([Permission.READ_EXTERNAL_STORAGE, Permission.WRITE_EXTERNAL_STORAGE])
    DATA_DIR = "/storage/emulated/0/Download/TransferData"
else:
...
# The same iPhone HTML GUI you liked!
HTML_GUI = """
<!DOCTYPE html>
<html>
<head>
...
class TransferApp(App):
    def build(self):
        ...
        # Title
        self.title_label = Label(text="Fast Transfer Server", font_size='30sp', bold=True, size_hint=(1, 0.2))
    ...
    def start_server(self, instance):
        if self.server_thread is None or not self.server_thread.is_alive():
            ...
            # Start FastAPI in a background thread so it doesn't freeze the Android UI
            self.server_thread = threading.Thread(target=self.run_uvicorn, args=(port,), daemon=True)
            self.server_thread.start()

    def run_uvicorn(self, port):
        # We run it with loops explicitly defined to prevent background thread async errors
